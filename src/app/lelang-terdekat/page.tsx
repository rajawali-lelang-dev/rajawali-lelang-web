        ...
        {upcomingLelang.length > 0 ? (
          upcomingLelang.map((item) => (
            <LelangCard
              ...
              type={item.type}
              status={item.status as "Lelang Aktif" | "Lelang Segera" | "Lelang Selesai"}
              jamLelang={'jamLelang' in item ? item.jamLelang : undefined}
              // MAPPING endPrice dari lib ke harga di Card
              harga={'endPrice' in item ? item.endPrice : undefined}
            />
          ))
        ) : (
          <div className="text-center py-16">
            <p className="text-lg text-neutral-600">Tidak ada lelang yang tersedia.</p>
          </div>
        )}
      </div>
    </div>
